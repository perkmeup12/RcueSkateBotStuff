...
for II=1:length(tailEImat)
%define filename time variables
tailEI=tailEImat(II);
d=700:100:1200;
for I=length(d)%1:length(d)
    delay=d(I);
    ...
    videotimevec=(0:(glideendframes(I)-startframes(I))-1)*timeperframe;
...
videocomparevector=interp1(videotimevec(58-startframes(I):end),videopitchangle(58:glideendframes(I))',time(pitchdiffindex),'spline');
pitchdiff(II,:)=abs(pitchanglesave(pitchdiffindex)-videocomparevector)./abs(videocomparevector);
