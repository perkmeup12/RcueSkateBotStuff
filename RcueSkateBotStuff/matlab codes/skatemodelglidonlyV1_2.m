...
delay=1100;
eval(['oldfold=cd([''C:\Users\Matt\Desktop\Skate\results\Cg12.41\leg 12.5\only2wgts\d'',num2str(delay)])'])
...
VehRinit=SensorGyrox(index(end));
Vehpitchinit=SensorRoll(index(end));
...
Vehpitchfin=SensorRoll(indexfin(end));
...
%%    Video data
    %change directory
    eval(['oldfold2=cd([''C:\Users\Matt\Desktop\Skate\results\Cg12.41\leg 12.5\only2wgts\d'',num2str(delay)])'])
    eval(['load(''d',num2str(delay),'videopositions'',''d',num2str(delay),'positions'');'])
    strctname=['d',num2str(delay),'positions'];
    
    %convert to inches
    eval(['noseloc=[',strctname,'.nose.x/',strctname,'.xcalibration,',strctname,'.nose.y/',strctname,'.ycalibration]';])
    eval(['dotloc=[',strctname,'.dot.x/',strctname,'.xcalibration,',strctname,'.dot.y/',strctname,'.ycalibration]';])
    
    figure
for ii=1:length(noseloc(:,1))
    vidorientationx1=[noseloc(:,1)';dotloc(:,1)'];
    vidorientationy1=[noseloc(:,2)';dotloc(:,2)'];
    plot(vidorientationx1,vidorientationy1,'r');
    ...
    %calculate pitch
    %start at 2
    videopitchoffset=5.5;
    videopitchangle=-(atand((noseloc(:,2)-dotloc(:,2))./(noseloc(:,1)-dotloc(:,1)))+videopitchoffset);

    %find corresponding time vector
    glidestartframe=17;
    tperframe=1/30;
    videotimevec=(0:1:(length(noseloc(:,2))-glidestartframe))*tperframe;
...
%Dive Angles
figure
plot(time,pitchanglesave,'b',comparetimevec,Vehpitch(indexcompare),'r',comparetimevec,ones(size(comparetimevec))*Vehpitchfin,'k--',...
    videotimevec,videopitchangle(glidestartframe:end),'g')
xlabel('time (s)')
ylabel('Pitch angle (deg)')
title('PitchAngle')
legend('Modeled','Experimental','Final Experimental','Video Capture')
...
title('Vehicle Inertial Velocities')
xlabel('time(s)')
ylabel('Velocity (m/s)')
