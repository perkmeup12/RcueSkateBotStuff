...
for I=1:length(d)
    ...
    ycalibration=xcalibration;%eval(['ycalibration=',strctname,'.ycalibration;'])
    ...
    vidpitchind=find(comparetimevec>.1 & comparetimevec<.2);
    pitchoffset=mean(Vehpitch(indexcompare(vidpitchind)))-mean(videopitchangle(startframes(I):startframes(I)+5));
    ...
    cgyoffset=sind(-cgoffsetangle)*mountcgdifx+cosd(-cgoffsetangle)*mountcgdify; %vertical distance above ground of cg at rest
    ...
    while running==1;
        ...
        if (time(end)-time(1))<=glidetimes(I) && y>0 && tailmountheight>0 &&mounty>0        %simulate vehicle glide phase
            % Skate is gliding%%%%%%%%%%%%%%%%%%
            ...
            %lift
            liftangle=-atand(v/u);
            V1=.7112; %horizontal velocity of flow test (m/s)
            V2=cosd(-pitchangle)*u+sind(-pitchangle)*v;
            ...
            %drag
            vertFd=-.5*dwater*dragcoeff*crossarea*(v)^2;                   %vertical drag force.
            %vertFd=0;
            ...
            if rigidheight<0
                ...
                tailmoment=deformdist*2*tailEI/taillength^2;
            else
                deformdist=0;
                tailforce=0;
                tailmoment=0;
...
%             rdot=-(liftforce*(Cgx-Cop)-tailforce*(x-xtip)+buoyforce*buoydist+munkmoment)/(IZZ+m66);

              udot=(sind(-pitchangle)*gravforce+sind(-pitchangle)*buoyforce+sind(-pitchangle)+...
                  sind(-pitchangle)*liftforce+cosd(-pitchangle)*horizFd)/(totalmass+m11);
              
              vdot=(cosd(-pitchangle)*gravforce+cosd(-pitchangle)*buoyforce+cosd(-pitchangle)*tailforce+...
                  cosd(-pitchangle)*liftforce+sind(-pitchangle)*horizFd)/(totalmass+m22);
              
              rdot=-(cosd(-pitchangle)*buoyforce*buoydist+cosd(-pitchangle)+...
                  cosd(-pitchangle)*liftforce*(Cgx-Cop)+munkmoment-tailmoment)/(IZZ+m66);
        ...
        horizfdsave=[horizfdsave,horizFd];
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %Position
    figure
    plot(xsave,ysave,'g',videocgloc(startframes(I):glideendframes(I),1),videocgloc(startframes(I):glideendframes(I),2),'k--');
    title(['Delay ',num2str(delay),' Vehicle Positions'])
    xlabel('X (m)')
    ylabel('Y (m)')
    legend('Modeled','Video Capture','Location','Best')
    hold on
    for ii=[1,round(length(xsave)/3),round(2*length(xsave)/3),length(xsave)]%1:round(length(xsave)/6):length(xsave)
        % read NACA file
