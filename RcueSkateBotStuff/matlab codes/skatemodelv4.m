...
while i<length(time)
    ...
    %if height of cg is 
    if y<=(-Tipy) && Vxrelative<=0 && thetadoubledot<0 && theta<0
        Vx=-Vtipx;
        Vy=-Vtipy;
...
%
figure
plot(time,Vxsave(2:end),'--',time,Vtipxsave(2:end),'-',time,Vxrelativesave(2:end),time,thetadoubledotsave(2:end)/max(thetadoubledotsave),'g','markersize',.5)
