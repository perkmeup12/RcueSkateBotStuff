...
while i<length(time)
    ...
    phaseshift=-pi/2;                                                               % to start the leg forward with zero speed at time t=0
    ...
    if y<(tiptravely) && Vxrelative<=0 && theta>(-legangletravel/4) && theta<0
            Vx=-Vtipx;
            Vy=-Vtipy;
...
hold on
tipreach=tiptravely*ones(size(time));
...
xlabel('time (s)')
ylabel('Velocity(m/s)')
legend('Vehicle','Vxtip','Vxrelative')
