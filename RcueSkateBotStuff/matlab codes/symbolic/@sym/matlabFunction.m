...
if any(missing)
    ...
    msg = 'symbolic:sym:matlabFunction:FreeVariable';
    if length(misvars) > 1
        msg = 'symbolic:sym:matlabFunction:FreeVariables';
    end
    error(message(msg, varnames));
end
...
for k = 1:N
    ...
    vk = strrep(vk,'_Var',''); % remove suffix from predefined names
