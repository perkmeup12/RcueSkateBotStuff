% HYPERGEOM   一般双曲線幾何関数
%
% HYPERGEIM(N, D, Z)は、Barnes の一般超幾何関数として知られ、
% j = length(N) です。k = length(D) のとき jFk として示される幾何関数 
% F(N, D, Z) を作成します。スカラー a, b, c に対して、
% HYPERGEOM([a, b], c, z)は、ガウス超幾何関数 2F1(a, b;c;z) となります。
% 
...
% はじめの2つの引数のどちらかは、単関数を評価する係数パラメータを得るための
% ベクトルになります。3番目の引数がベクトルの場合、関数は点として評
% 価されます。すべての引数が数値の場合、結果は数値になり、すべての引数が
% シンボリック式の場合、結果はシンボリック式になります。
% 
% Abramowitz と Stegun の Handbook of Mathematical Functions の 15章を
% 参照してください。
%
% 例題：
%    syms a z
