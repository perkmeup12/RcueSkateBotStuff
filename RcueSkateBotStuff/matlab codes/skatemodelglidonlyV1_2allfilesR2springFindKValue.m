...
springKmat=[10:10:200];
for II=1:length(springKmat)
...
for I=length(d)
    ...
    while running==1;
        ...
        if (time(end)-time(1))<=glidetimes(I)%y>0 &&mounty>0                          %simulate vehicle glide phase
            ...
            springlength=videocgloc(58,2)+sind(videopitchangle(58))*taildist;                %height of tail mount above ground
            springK=springKmat(II);
            
            springdisplacement=tailmountheight-springlength;
            if springdisplacement<0
                tailforce=-springK*springdisplacement;
            else
                tailforce=0;
            end
            xtip=tailmountx;
    ...
    hold on
    plot(videotimevec(58-startframes(I))*[1,1],[min(pitchanglesave),max(pitchanglesave)],'--k')
    xlabel('time (s)')
    ...
    hold on
    for ii=[1,100,200,300,400,length(xsave)]%1:round(length(xsave)/6):length(xsave)
        % read NACA file
...
pitchdiffindex=find(time>videotimevec(58-startframes(I)));
videocomparevector=interp1(videotimevec(58-startframes(I)+1:end),videopitchangle(58:glideendframes(I))',time(pitchdiffindex),'spline');
pitchdiff(II,:)=abs(pitchanglesave(pitchdiffindex)-videocomparevector);
pause(1)
end

for II=1:length(springKmat)
pitchsums(II)=sum(pitchdiff(II,:));
end
[val,Ind]=min(pitchsums);
bestk=springKmat(Ind)
