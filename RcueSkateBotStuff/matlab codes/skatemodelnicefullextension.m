...
while i<length(time)
    ...
    phaseshift=-pi/4;                                                               % to start the leg forward with zero speed at time t=0
    ...
    if y<=(maxtipy) && Vxrelative<=0 && theta>(-legangletravel/4) && theta<0
        Vx=-Vtipx;
        Vy=-Vtipy;
