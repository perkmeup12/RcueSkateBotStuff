...
nutoffset=.0254;                                %distance from origin of tip movement to bottom of nut
tipdiameter=.0254/2;                             %thickness of leg
...
waitperiod=2%d(ii);                      %period of waiting between strokes
...
mountangle=12.5;                    %angle of leg mount with respect to the body
alphaleg=mountangle+AoA;            %angle of leg plane(degrees)
...
Tipysave=[];
Tipxsave=[];
Vertaddedmassforcesave=[];
Vertaddedmassforce=0;
%maxtipy=.0373;
...
while i<length(time)
    ...
    if strokeindex==1;
        ...
        Tipy=sind(alphaleg)*((theta)*C)-0.0185+nutoffset-tipdiameter;
        Tipx=cosd(alphaleg)*theta*C;
        %----------------------------------------
        ...
        if y<=(-Tipy) && Vxrelative<=0 && thetadoubledot<0 && y>=0
            ...
            Ay=0;%sind(alphaleg)*(C*thetadoubledot);
            x=x+Vx*dt;
            y=y+Vy*dt;
            phase=1;
        ...
        elseif y>0
            ...
            %Vertaddedmass=2*integral(radfunC,0,a);
            %Vertaddedmassforce=0;
            
            Vertaddedmass=dwater*pi*a^2;
            Vertaddedmassforce=0;  %added mass force on a flat plate
            
            %accelerations
            Ax=(horizFd/(totalmass));                              %horizontal acceleration
            Ay=(buoyforce+gravforce+vertFd+liftforce+Vertaddedmassforce)/(totalmass+Vertaddedmass); %vertical acceleration
        ...
        Tipxsave=[Tipxsave,Tipx];
        Vertaddedmassforcesave=[Vertaddedmassforcesave,Vertaddedmassforce];
        ...
        if y>0
            ...
            b=.07;              %vertical radius(m)
            c=.305;             %side radius (m)
            radfunC= @(intx) pi*(1-intx.^2/a)*c;
            %Vertaddedmass=(dwater)*integral(radfunC,-a,a);
            %Vertaddedmassforce=0;
